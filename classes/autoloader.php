...
class autoloader
{
	...
	const defaultCacheFileName = '%s.atoum.shell.cache';
    ...
    public function __construct(array $namespaces = array(), array $namespaceAliases = array(), $classAliases = array())
    {
        ...
        foreach ($namespaceAliases ?: array('mageekguy\atoum\shell' => 'atoum\shell') as $alias => $target)
        {
            $this->addNamespaceAlias($alias, $target);
        }

        $classAliases = $classAliases ?: array(
            'atoum\shell' => 'atoum\shell\runner',
            'mageekguy\atoum\shell' => 'atoum\shell\runner'
        );
