...
class command implements killable
{
    ...
    public function getIncrementalStdout()
    {
        $out = $this->stdOut;
        $this->stdOut = '';
    ...
    public function run($stdin = '')
    {
        if ($this->processus !== null)
        {
            throw new cli\command\exception('Unable to run \'' . $this . '\' because is currently running');
        }
        ...
        {
            throw new cli\command\exception('Unable to run \'' . $this . '\'');
        }
        ...
        if (isset($this->streams[0]) === true)
        {
            while ($stdin != '')
            {
                ...
                {
                    throw new cli\command\exception('Unable to send \'' . $stdin . '\' to \'' . $this . '\'');
                }
    ...
    {
        if ($this->processus !== null && $this->isRunning())
        {
            return proc_terminate($this->processus, $signal);
        }
